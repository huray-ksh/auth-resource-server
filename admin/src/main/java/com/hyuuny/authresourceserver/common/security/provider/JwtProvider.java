package com.hyuuny.authresourceserver.common.security.provider;
...
import com.hyuuny.authresourceserver.common.AuthResourceAppConstant;
...
import com.hyuuny.authresourceserver.domain.service.RefreshTokenService;
import com.hyuuny.authresourceserver.domain.service.UserDomainService;
import com.hyuuny.authresourceserver.infrastructure.create.RedisService;
...
import static com.hyuuny.authresourceserver.code.RefreshTokenType.SERVICE_ADMIN;
import static com.hyuuny.authresourceserver.code.RefreshTokenType.SERVICE_APP;
...
public class JwtProvider {
    ...
    public Authentication verifyAccessToken(String jwtToken, Scope scope) {
        ...
        UserAdapter userDetail = getUserDetails(userIdString);
    ...
    private UserAdapter getUserDetails(String userIdString) {
        String formattedRedisUserKey = getRedisAdminUserKey(userIdString);
        Object userDetailObject = redisService.getObject(formattedRedisUserKey);
        if (!(userDetailObject instanceof UserAdapter)) {
            UserAdapter userDetail = userDomainService.getUserDetails(Long.valueOf(userIdString));
            redisService.setObjectAfterForceKeyExpiration(formattedRedisUserKey, userDetail, (accessTokenValidDuration - 1000));
            return userDetail;
        }
        log.debug("####### Redis object = {}", userDetailObject);
        return (UserAdapter) userDetailObject;
    }

    private String getRedisAdminUserKey(String userIdString) {
        String redisUserKey = AuthResourceAppConstant.CRATE_AUTH_RESOURCE_APP_KEY_SYS_ADMIN;
    ...
    public AuthenticationDto.UserWithToken issueLoginToken(Long userId) {
        UserAdapter userDetails = userDomainService.getUserDetails(userId);
        Date now = new Date();
        Date expirationDate = getExpiredDate(now, Scope.ACCESS);
        Date refreshExpirationDate = getExpiredDate(now, Scope.REFRESH);

        String accessTokenString = createToken(userId, Scope.ACCESS, now, expirationDate);
        String refreshTokenString = createToken(userId, Scope.REFRESH, now, refreshExpirationDate);
    ...
    private String createToken(Long userId, Scope scope, Date now, Date expirationDate) {
    ...
    public AuthenticationDto.UserWithToken reissueLoginToken(String refreshToken) {
        ...
        try {
            ...
            refreshTokenService.verifyUserRefreshToken(SERVICE_ADMIN, Long.valueOf(userIdString), issuerId);
    ...
    public void deleteRefreshToken(String refreshToken) {
        Claims body = this.validateSignedTokenAndGetClaims(refreshToken);
        String userIdString = body.getSubject();
        refreshTokenService.deleteRefreshToken(SERVICE_ADMIN, Long.valueOf(userIdString));
    }
